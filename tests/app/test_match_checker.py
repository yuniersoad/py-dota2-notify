...
async def test_check_new_matches_no_new_matches(mock_db_client, mock_opendota_client, mock_telegram_client):
    ...
    mock_user.following = False
    
    mock_followed_player = MagicMock()
    mock_followed_player.user_id = 123
    mock_followed_player.id = "456"
    mock_followed_player.name = "ProPlayer"
    mock_followed_player.last_match_id = 7890
    
    mock_db_client.get_friends_async = AsyncMock(return_value=[mock_followed_player])
...
@pytest.mark.asyncio
async def test_check_new_matches_no_followed_players_follows_himself(mock_db_client, mock_opendota_client, mock_telegram_client):
    # Arrange
    mock_user = MagicMock()
    mock_user.id = "123"
    mock_user.name = "Test User"
    mock_user.user_id = 123
    mock_user.telegram_chat_id = "chat123"
    mock_user.following = True
    
    mock_db_client.get_friends_async = AsyncMock(return_value=[])
    ...
    # Assert
    mock_opendota_client.get_player_matches.assert_called_once_with('123', 1)
    mock_telegram_client.send_message.assert_called_once_with("chat123", "Test User ✅ Won a match as Anti-Mage with KDA 10/2/15. Match duration: 40m 0s. Match details: https://www.dotabuff.com/matches/7891")
    mock_db_client.update_last_match_id.assert_called_once_with(123, 123, 7891)
...
async def test_check_new_matches_new_match_found_won(mock_db_client, mock_opendota_client, mock_telegram_client):
    ...
    mock_followed_player = MagicMock()
    mock_followed_player.id = "456"
    mock_followed_player.following = True
    mock_followed_player.user_id = 123
    mock_followed_player.name = "ProPlayer"
    mock_followed_player.last_match_id = 7890
    ...
    mock_db_client.update_last_match_id.assert_called_once_with(123, 456, 7891)
